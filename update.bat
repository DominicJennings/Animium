...
if exist package-lock.json ( del package-lock.json || echo Error deleting the package-lock.json file. && goto errorSetting )
:errorSetting
...
if %ERROR_OCCURED_ONCE%==y ( if %ERROR_OCCURED_TWICE%==y ( echo There was an error while peforming two tasks. so the update will be skipped. && pause & exit ) else ( echo There was an error while peforming a task. so the update will be skipped. && pause & exit ) )
